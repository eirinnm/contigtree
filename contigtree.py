...
parser = argparse.ArgumentParser(description='''Generate a tree of contigs from a Minia contig file.
                                 ...
                                 The sequences are printed to stdout in FASTA format while the tree is printed to stderr.''')
...
def linehunter(target, filename):
    ...
    while foundline != target:
        ...
        parts = text.split()
        if not parts[0].startswith(b'>'): raise ValueError("Malformed header line")
...
#    print('\t'*depth,ID)
    ...
    pos, text, seq = linehunter(ID, filename)
    if flip_flag: 
        seq = reverse_complement(seq)
    this_segment = Segment(ID, flip_flag, seq)
    segments.append(this_segment)
    parts = text.split()
    if not parts[0].startswith('>'): raise ValueError("Malformed header line")
    branches = []
